...
namespace AutoUpdateServer_WEB
{
	public class UpdateTypeKey 
	{
		...
		public enStreamStatus StreamStatus = enStreamStatus.Closed;
		...
		public UpdateTypeKey(string updateType)
		{
	...
	public class fnc
	{
		...
		/// Update 설정 xml 파일명
		/// </summary>
		public static readonly string UpdateTypeXmlFileName = "UpdateDataSet.xml";

		/// <summary>
		/// 임시폴더 이름
		/// </summary>
		public static readonly string TempDiretoryName = "_Temp";

		/// <summary>
		/// 히스토리 폴더 이름
		/// </summary>
		public static readonly string HistoyDiretoryName = "_History";
		...
		public static void FileStream_Close(ref FileStream fs)
        {
			try
			{
				fs.Flush();
		...
		/// <returns></returns>
		public static string Get_Crc32(string filepath)
		{
		...
		public static string TempFile_init(string filename)
		{
			...
			if(File.Exists(filename))
			{
				...
				foreach(FileInfo f in fi.Directory.GetFiles())
				{
					try
					{
						f.Delete();
				...
				while(File.Exists(filename))
				{
					...
					filename = string.Format("{0}\\{1}({3}){2}", fi.DirectoryName, fi.Name, fi.Extension, cnt);
				}
