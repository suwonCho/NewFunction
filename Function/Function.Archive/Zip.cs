...
namespace Function.Archive
 {
	/// <summary> 
	/// Zip 파일을 만들거나 파일을 추가 한다. <para/>
	/// 생성 : Function.Archive.ZipStorer z = ZipStorer.Create(@"파일명", "test"); or Open
	/// </summary> 
	public class ZipStorer : IDisposable 
	 { 
		 ...
		 public delegate void delZipProgress(long total, long curr);

		 event delZipProgress _onZipProgress;

		 /// <summary>
		 /// 파일별 압축 진행 상황 델리게이트
		 /// </summary>
		 public event delZipProgress OnZipProgress
		 {
			 add { _onZipProgress += value; }
			 remove { _onZipProgress -= value; }
		 }

		 public bool Canceled = false;
		 public bool ReqCancel = false;
		...
		{ 
			 ...
			 { 
				 ...
				 if (_onZipProgress != null) _onZipProgress(_source.Length, totalRead);
