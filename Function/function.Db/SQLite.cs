...
namespace function.Db
{
	...
	public class SQLite
	{
        ...
        /// <param name="CreateDb">DB File이 없으면 생성</param>
        public SQLite(string dbfile, bool CreateDb = false)
        {            
			Set_ConnectionString(dbfile, CreateDb);
		}
		...
		/// </summary>
		/// <param name="dbfile">DB File 경로</param>
		public void Set_ConnectionString(string dbfile, bool CreateDb = false)
        {
			connectionString = string.Format("Data Source={0}", dbfile);

            if (CreateDb && !File.Exists(dbfile))
            {
                FileStream writer = new FileStream(dbfile, FileMode.CreateNew, FileAccess.Write, FileShare.Write);
                writer.Write(Properties.Resources.EmptyDBFile, 0, Properties.Resources.EmptyDBFile.Length);
                writer.Close();
            }
