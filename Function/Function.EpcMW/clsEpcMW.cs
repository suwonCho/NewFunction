...
namespace Function.EpcMW
{
	...
	/// </summary>
	public class clsEpcMW : IDisposable
	{
		...
		ALEService Ale;		
		Function.Comm.clsSocketServer clsSvr;
		private delegate void delClientConected (string strMsg);
		private delClientConected evtClientConected;
		...
		private void StratEventServer(int _intPort)
		{
			...
			{
				intPort = _intPort;
				clsSvr = new Function.Comm.clsSocketServer(intPort, 1);
                clsSvr.evtClentConnect = new Function.Comm.clsSocketServer.delClientConnect(evtClientConnected);
                clsSvr.evtReceiveRequest = new Function.Comm.clsSocketServer.delReceiveRequest(evtReceiveRequest);
				clsSvr.Start();
