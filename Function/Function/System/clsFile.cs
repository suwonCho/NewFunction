...
namespace Function
{
	public partial class system
	{
		...
		public class clsFile
		{
			...
			/// <param name="strFilePath"></param>
			/// <returns></returns>
			public static string[] FileSplitPath_Name(string strFilePath)
			{
				try
				{
					FileInfo fi = new FileInfo(strFilePath);

					return new string[] { fi.DirectoryName, fi.Name };

					//return System.Diagnostics.FileVersionInfo.GetVersionInfo(strFilePath).FileName;
				}
				catch
				{
					return null;
			...
			/// <param name="strFilePath"></param>
			/// <returns></returns>
			public static string FileGetVersion(string strFilePath)
			{
				return System.Diagnostics.FileVersionInfo.GetVersionInfo(strFilePath).FileVersion;
			}
			...
			/// <param name="strFilePath"></param>
			/// <returns></returns>
			public static void FileDelete(string strFilePath)
			{
			...
			/// <param name="strFilePath"></param>
			/// <returns></returns>
			public static void FileCopy(string strSourceFilePath, string strDestFilePath)
			{
				if (File.Exists(strSourceFilePath))
				{
					string[] str = FileSplitPath_Name(strDestFilePath);

					if (!FolderExists(str[0]))
					{
						FolderCreate(str[0]);
					}

					File.Copy(strSourceFilePath, strDestFilePath);
			...
			/// <param name="strFilePath"></param>
			/// <returns></returns>
			public static void FileMove(string strSourceFilePath, string strDestFilePath)
			{
				if (File.Exists(strSourceFilePath))
				{
					string[] str = FileSplitPath_Name(strDestFilePath);
			...
			public static string FileName_Check(string fullfilename)
			{
				...
				while (!bconfirm)
				{
					if (i == 0)
					{
						...
						ext = fi.Extension;
						file = fi.Name.Substring(0, fi.Name.Length - ext.Length);
						
					}
					else
					{
						fi = new FileInfo(string.Format("{0}\\{1}({2}){3}", folder, file, i, ext));
					}

					if (!fi.Exists)  bconfirm = true;
			...
			/// <returns></returns>
			public static string [] FileSelect(string strInitialFilePath, string strFilter, bool isMultiselect, IWin32Window iWin )
			{
				...
				if (File.Exists(strInitialFilePath))
				{
					FileInfo fi = new FileInfo(strInitialFilePath);
			...
			/// <returns></returns>
			public static string FolderName_Check(string FolderPath)
			{
				int i = 0;
				bool bconfirm = false;								
				...
				while (!bconfirm)
				{
					if(!Directory.Exists(folder))
						...
						folder = string.Format("{0}({1})", FolderPath, i);
			...
			/// </summary>
			/// <param name="strPath"></param>
			public static void FolderDelete(string strPath)
			{
				if (!FolderExists(strPath)) return;
				...
				//폴더삭제
				foreach(string fld in Directory.GetDirectories(strPath))
				{
					FolderDelete(fld);
				}
			...
			public static long FolderFileCount(string FolderPath)
			{
				long cnt = 0;

				DirectoryInfo di = new DirectoryInfo(FolderPath);
				cnt = folderFileCount(di);

				return cnt;
			}

			static long folderFileCount(DirectoryInfo di)
			{
				long cnt = di.GetFiles().Length;
			...
			/// <param name="fi"></param>
			/// <returns></returns>
			public static string Get_Crc32(FileInfo fi)
			{			
				try
				{
					...
					for (int iTotal = 0; iTotal < Totals.Length; iTotal++)
					{
						...
						Stream xStream = fi.OpenRead();

						int iRead;
						
						while ( 0 < (iRead = xStream.Read(xBuf, 0, xBuf.Length)))
						{
							iCrc[iTotal]
								= Crc32(iCrc[iTotal], xBuf, 0, iRead);						
						}
					...
					{					
						if (iTotal < 1)
							strCrc32 = iCrc[iTotal].ToString("x8");
						else
							if (iCrc[iTotal] != iCrc[0])
								strCrc32 = iCrc[iTotal].ToString("x8") + "(" + iTotal.ToString() + ") WHOOPS";
					}
			...
			// NOTE: No PolyVals dynamic build! Weighing dynamic generation code bytes=<whatever>
			// versus 1024 static bytes, we suggest the savings(if any) of dynamic generation
			// are not significant and static storage never has to be dynamically generated (grin).
