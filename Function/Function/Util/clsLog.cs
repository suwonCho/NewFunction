...
namespace Function.Util
{
	...
	public class Log
	{
		...
		public Log(string _Directory, string FileName,int _DelDays, bool _OnOff)
		{
			try
			{
				/// 폴더 존재 여부 검사한다.
		...
		/// <returns></returns>
		public bool WLog(string format, params object[] args)
		{
			return WLog(string.Format(format, args));
		}
...
#region static부
		...
		/// <param name="dr"></param>
		/// <returns></returns>
		public static string datarow_singleLog(DataRow dr)
		{
			int iIndex = 0;
			...
			{
				log += string.Format("[{0}]{1}", c.ColumnName, dr[iIndex]);
				iIndex++;
			}
		...
		/// <returns></returns>
		public static string datarow_ModifyLog(DataRow sourcerow, DataRow modifyrow)
		{
			int iIndex = 0;
			string log = string.Empty;
			...
			{
				log += string.Format("[{0}]{1}->{2}", c.ColumnName, sourcerow[iIndex], modifyrow[iIndex]);
				iIndex++;
			}
